...
Default backend: ~/.agent-harness/credentials.json with mode 600.
...
_KEYRING_SERVICE = "agent-harness"
...
def _get_config_dir() -> Path:
    """Return the agent-harness config directory."""
...
def _creds_path() -> Path:
    return _get_config_dir() / _CREDS_FILE_NAME
...
def _obfuscation_key() -> bytes:
    ...
    seed = str(Path.home()).encode() + b"agent-harness-v1"
