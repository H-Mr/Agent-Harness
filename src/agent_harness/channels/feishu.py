...
def _extract_share_card_content(content_json: dict, msg_type: str) -> str:
    ...
    if msg_type == "share_chat":
        parts.append("[shared chat: %s]" % content_json.get("chat_id", ""))
    elif msg_type == "share_user":
        parts.append("[shared user: %s]" % content_json.get("user_id", ""))
    elif msg_type == "interactive":
        parts.extend(_extract_interactive_content(content_json))
    elif msg_type == "share_calendar_event":
        parts.append("[shared calendar event: %s]" % content_json.get("event_key", ""))
    elif msg_type == "system":
        parts.append("[system message]")
    elif msg_type == "merge_forward":
        parts.append("[merged forward messages]")

    return "\n".join(parts) if parts else "[%s]" % msg_type
...
def _extract_interactive_content(content: dict) -> list[str]:
    ...
    if "title" in content:
        ...
        if isinstance(title, dict):
            ...
            if title_content:
                parts.append("title: %s" % title_content)
    ...
    if header:
        ...
        if isinstance(header_title, dict):
            ...
            if header_text:
                parts.append("title: %s" % header_text)
        ...
        for row in block["content"]:
            ...
            for el in row:
                ...
                if tag in ("text", "a"):
                    ...
                    texts.append("@%s" % el.get("user_name", "user"))
                elif tag == "code_block":
...
class FeishuChannel(BaseChannel):
    ...
    async def stop(self) -> None:
        ...
        Note: lark.ws.Client does not expose a stop method; simply exiting the
        program will close the client.
    ...
    def _parse_md_table(cls, table_text: str) -> dict | None:
        ...
        rows = [
            [cls._strip_md_formatting(c) for c in split(_line)] for _line in lines[2:]
        ]
        columns = [
            {"tag": "column", "name": "c%d" % i, "display_name": h, "width": "auto"}
        ...
        return {
            ...
            "columns": columns,
            "rows": [
                {"c%d" % i: r[i] if i < len(r) else "" for i in range(len(headers))}
                for r in rows
            ],
        }
    ...
    def _split_headings(self, content: str) -> list[dict]:
        ...
        for m in self._CODE_BLOCK_RE.finditer(content):
            ...
            protected = protected.replace(m.group(1), "\x00CODE%d\x00" % (len(code_blocks) - 1), 1)
        ...
        for m in self._HEADING_RE.finditer(protected):
            ...
            display_text = "**%s**" % text if text else ""
            elements.append(
                {
                    "tag": "div",
                    "text": {
        ...
        for i, cb in enumerate(code_blocks):
            for el in elements:
                if el.get("tag") == "markdown":
                    el["content"] = el["content"].replace("\x00CODE%d\x00" % i, cb)

        return elements or [{"tag": "markdown", "content": content}]

    # -- Smart format detection --
    _COMPLEX_MD_RE = re.compile(
        r"```" r"|^\|.+\|.*\n\s*\|[-:\s|]+\|" r"|^#{1,6}\s+", re.MULTILINE,
    )
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                if response.success():
                    ...
                    logger.debug(
                        "Uploaded image %s: %s", os.path.basename(file_path), image_key
                    )
                    return image_key
                else:
                    logger.error(
    ...
    def _download_image_sync(
        ...
        try:
            ...
            if response.success():
                ...
                logger.error(
                    "Failed to download image: code=%s, msg=%s",
                    response.code,
                    response.msg,
                )
                return None, None
    ...
    async def _download_and_save_media(
        ...
        """
        loop = asyncio.get_running_loop()
        media_dir = Path.home() / ".agent-harness" / "media" / "feishu"
        ...
        if msg_type == "image":
            ...
            if file_key and message_id:
                ...
                if msg_type == "audio" and not filename.endswith(".opus"):
                    filename = "%s.opus" % filename

        if data and filename:
            file_path = media_dir / filename
            file_path.write_bytes(data)
            logger.debug("Downloaded %s to %s", msg_type, file_path)
            return str(file_path), "[%s: %s]" % (msg_type, filename)

        return None, "[%s: download failed]" % msg_type
    ...
    def _reply_message_sync(
        ...
        try:
            ...
            logger.error(
                "Error replying to Feishu message %s: %s", parent_message_id, e
            )
            return False
            ...
            if msg.content and msg.content.strip():
                ...
                if fmt == "text":
                    text_body = json.dumps(
                        {"text": msg.content.strip()}, ensure_ascii=False
                    )
    ...
    async def _on_message(self, data: Any) -> None:
        ...
        try:
            ...
            if msg_type == "text":
                ...
                content_parts.append(MSG_TYPE_MAP.get(msg_type, "[%s]" % msg_type))
