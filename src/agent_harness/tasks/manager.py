...
from agent_harness.tasks.types import TaskRecord, TaskStatus, TaskType
...
class BackgroundTaskManager:
    ...
    def __init__(self, tasks_dir: Path | None = None) -> None:
        self._tasks_dir = tasks_dir or (Path.home() / ".agent-harness" / "tasks")
    ...
    async def create_agent_task(
        ...
        *,
        ...
        if command is None:
            ...
            cmd = ["agent-harness", "--api-key", effective_api_key]
            if model:
    ...
    async def _start_process(self, task_id: str) -> asyncio.subprocess.Process:
        ...
        process = await _create_shell_subprocess(
            task.command,
            cwd=task.cwd,
...
def get_task_manager(tasks_dir: Path | None = None) -> BackgroundTaskManager:
    """Return the singleton task manager.

    Optionally provide *tasks_dir* on first call to override the default
    output directory (``~/.agent-harness/tasks``).
    """
    global _DEFAULT_MANAGER, _DEFAULT_MANAGER_KEY
    effective_dir = tasks_dir or (Path.home() / ".agent-harness" / "tasks")
    key = str(effective_dir.resolve())
    if _DEFAULT_MANAGER is None or _DEFAULT_MANAGER_KEY != key:
        _DEFAULT_MANAGER = BackgroundTaskManager(tasks_dir=effective_dir)
        _DEFAULT_MANAGER_KEY = key
...
async def _create_shell_subprocess(
    command: str,
    *,
    cwd: str | Path,
    stdin: int | None = None,
    ...
    """Simple subprocess creation — no sandboxing or OpenHarness-specific shell resolution.

    On platforms where ``bash`` is available it is preferred so that login-shell
    context (``.bashrc``, ``.profile``) is loaded.  Falls back to the system
    default shell otherwise.
    """
