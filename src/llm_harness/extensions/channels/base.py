...
class BaseChannel(ABC):
    ...
    to integrate with the agent-harness message bus.
    """
    ...
    def is_allowed(self, sender_id: str) -> bool:
        ...
        if isinstance(cfg, dict):
            allow_list = cfg.get("allow_from", [])
        else:
            allow_list = getattr(cfg, "allow_from", [])
        if not allow_list:
    ...
    async def on_connect(self, session_key: str) -> None:
        """Called when a session connects via this channel."""
        pass

    async def on_disconnect(self, session_key: str) -> None:
        """Called when a session disconnects from this channel."""
        pass

    async def on_message(self, msg: OutboundMessage) -> None:
        """Called when a message is routed through this channel."""
        pass
