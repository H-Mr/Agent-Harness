...
class WeChatChannel(BaseChannel):
    ...
    def default_config(cls) -> dict[str, Any]:
        return WeixinConfig().model_dump()
    ...
    def _get_state_dir(self) -> Path:
        ...
        if self.config.state_dir:
            ...
            d = Path.home() / ".agent-harness" / "weixin"
    ...
    def _load_state(self) -> bool:
        ...
        try:
            ...
            logger.warning("Failed to load WeChat state: %s", e)
            return False
    ...
    def _save_state(self) -> None:
        ...
        try:
            ...
            logger.warning("Failed to save WeChat state: %s", e)
    ...
    async def _api_get(
        ...
        *,
        ...
        url = "%s/%s" % (self.config.base_url, endpoint)
    ...
    async def _fetch_qr_code(self) -> tuple[str, str]:
        ...
        if not qrcode_id:
            raise RuntimeError("Failed to get QR code from WeChat API: %s" % data)
            ...
            while self._running:
                ...
                if status == "confirmed":
                    ...
                    if token:
                        ...
                        self._save_state()
                        logger.info(
                            "WeChat login successful! bot_id=%s user_id=%s",
                            bot_id,
                            user_id,
                        )
                        return True
                    ...
                    if refresh_count > MAX_QR_REFRESH_COUNT:
                        logger.warning(
                            "QR code expired too many times (%d/%d), giving up.",
                            refresh_count - 1,
    ...
    def _print_qr_code(url: str) -> None:
        try:
            ...
            logger.info("QR code URL (install 'qrcode' for terminal display): %s", url)
            print("\nLogin URL: %s\n" % url)
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Authenticate via login() first.")
                self._running = False
                return
    ...
    def _assert_session_active(self) -> None:
        ...
        if remaining > 0:
            ...
            raise RuntimeError(
                "WeChat session paused, %d min remaining (errcode %d)"
                % (remaining_min, ERRCODE_SESSION_EXPIRED)
            )
    ...
    async def _poll_once(self) -> None:
        ...
        if remaining > 0:
            logger.warning(
                "WeChat session paused, waiting %d min before next poll.",
        ...
        if is_error:
            if errcode == ERRCODE_SESSION_EXPIRED or ret == ERRCODE_SESSION_EXPIRED:
                self._pause_session()
                remaining_s = self._session_pause_remaining_s()
                logger.warning(
                    "WeChat session expired (errcode %s). Pausing %d min.",
                    errcode,
                    max((remaining_s + 59) // 60, 1),
                )
                return
            raise RuntimeError(
                "getUpdates failed: ret=%s errcode=%s errmsg=%s"
                % (ret, errcode, data.get("errmsg", ""))
            )
    ...
    async def _process_message(self, msg: dict) -> None:
        ...
        if not msg_id:
            msg_id = "%s_%s" % (msg.get("from_user_id", ""), msg.get("create_time_ms", ""))
        ...
        for item in item_list:
            ...
            if item_type == ITEM_TEXT:
                ...
                if voice_text:
                    content_parts.append("[voice] %s" % voice_text)
                else:
                    file_path = await self._download_media_item(voice_item, "voice")
                    if file_path:
                        content_parts.append("[voice]\n[Audio: source: %s]" % file_path)
                        media_paths.append(file_path)
                    else:
                        content_parts.append("[voice]")
                ...
                if file_path:
                    content_parts.append("[file: %s]\n[File: source: %s]" % (file_name, file_path))
                    media_paths.append(file_path)
                else:
                    content_parts.append("[file: %s]" % file_name)

            elif item_type == ITEM_VIDEO:
                video_item = item.get("video_item") or {}
                file_path = await self._download_media_item(video_item, "video")
                if file_path:
                    content_parts.append("[video]\n[Video: source: %s]" % file_path)
                    media_paths.append(file_path)
                else:
                    content_parts.append("[video]")
        ...
        logger.info(
            "WeChat inbound: from=%s items=%s bodyLen=%d",
            from_user_id,
    ...
    async def _download_media_item(
        ...
        try:
            ...
            cdn_url = (
                ...
                % (self.config.cdn_base_url, quote(encrypt_query_param))
            )
            ...
            if aes_key_b64 and data:
                ...
                logger.debug("No AES key for %s item, using raw bytes", media_type)

            if not data:
                return None

            media_dir = Path.home() / ".agent-harness" / "media" / "weixin"
            media_dir.mkdir(parents=True, exist_ok=True)
            ext = _ext_for_type(media_type)
            if not filename:
                ts = int(time.time())
                h = abs(hash(encrypt_query_param)) % 100000
                filename = "%s_%d_%d%s" % (media_type, ts, h, ext)
            safe_name = os.path.basename(filename)
            file_path = media_dir / safe_name
            file_path.write_bytes(data)
            logger.debug("Downloaded WeChat %s to %s", media_type, file_path)
            return str(file_path)

        except Exception as e:
            logger.error("Error downloading WeChat media: %s", e)
            return None
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            logger.warning("WeChat send blocked: %s", e)
            return
        ...
        if not ctx_token:
            logger.warning(
                "WeChat: no context_token for chat_id=%s, cannot send",
                msg.chat_id,
            )
            return
        ...
        for media_path in msg.media or []:
            try:
                ...
                logger.error("Failed to send WeChat media %s: %s", media_path, e)
                await self._send_text(
                    msg.chat_id, "[Failed to send: %s]" % filename, ctx_token,
                )
    ...
    async def _send_text(
        ...
        client_id = "ah-%s" % uuid.uuid4().hex[:12]
    ...
    async def _send_media_file(
        ...
        p = Path(media_path)
        if not p.is_file():
            raise FileNotFoundError("Media file not found: %s" % media_path)
        ...
        logger.debug("WeChat getuploadurl response: %s", upload_resp)

        upload_param = upload_resp.get("upload_param", "")
        if not upload_param:
            raise RuntimeError("getuploadurl returned no upload_param: %s" % upload_resp)
        ...
        cdn_upload_url = (
            ...
            % (self.config.cdn_base_url, quote(upload_param), quote(file_key))
        )
        logger.debug(
            "WeChat CDN POST url=%.80s ciphertextSize=%d", cdn_upload_url, len(encrypted_data)
        )
        ...
        if not download_param:
            raise RuntimeError(
                ...
                "status=%s headers=%s" % (cdn_resp.status_code, dict(cdn_resp.headers))
            )
        logger.debug("WeChat CDN upload success for %s, got download_param", p.name)
        ...
        if errcode and errcode != 0:
            raise RuntimeError(
                "WeChat send media error (code %s): %s" % (errcode, data.get("errmsg", ""))
            )
        logger.info("WeChat media sent: %s (type=%s)", p.name, item_key)
...
def _parse_aes_key(aes_key_b64: str) -> bytes:
    ...
    * ``base64(hex string of 16 bytes)``  -> file / voice / video
    ...
    raise ValueError(
        "aes_key must decode to 16 raw bytes or 32-char hex string, got %d bytes" % len(decoded)
    )
