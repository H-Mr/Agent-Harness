"""Tests for MCP tool wrapper and connection management.

Adapted for agent-harness: MCPToolWrapper now uses Pydantic input models.
MCPServerConfig replaced with SimpleNamespace (not available in agent-harness schema).
"""
...
def _make_tool_def(name: str, input_schema: dict | None = None) -> SimpleNamespace:
...
def _make_mcp_cfg(**kwargs) -> SimpleNamespace:
    """Create a SimpleNamespace that looks like MCPServerConfig for the test."""
    defaults = dict(
        type="stdio",
        command="fake",
        args=[],
...
def test_wrapper_preserves_non_nullable_unions() -> None:
    ...
    raw_schema = wrapper.input_model.model_json_schema()
...
async def test_execute_returns_text_blocks() -> None:
    ...
    tool_def = _make_tool_def("demo", schema)
    wrapper = MCPToolWrapper(SimpleNamespace(call_tool=call_tool), "test", tool_def)

    args = wrapper.input_model(value=1)
    result = await wrapper.execute(args, ToolExecutionContext(cwd=__import__("pathlib").Path.cwd()))
...
async def test_execute_returns_timeout_message() -> None:
    ...
    tool_def = _make_tool_def("demo", {"type": "object", "properties": {}})
    wrapper = MCPToolWrapper(SimpleNamespace(call_tool=call_tool), "test", tool_def, tool_timeout=0.01)

    args = wrapper.input_model()
    result = await wrapper.execute(args, ToolExecutionContext(cwd=__import__("pathlib").Path.cwd()))
...
async def test_execute_re_raises_external_cancellation() -> None:
    ...
    tool_def = _make_tool_def("demo", {"type": "object", "properties": {}})
    wrapper = MCPToolWrapper(SimpleNamespace(call_tool=call_tool), "test", tool_def, tool_timeout=10)

    args = wrapper.input_model()
    task = asyncio.create_task(
        wrapper.execute(args, ToolExecutionContext(cwd=__import__("pathlib").Path.cwd()))
    )
    await started.wait()
...
async def test_connect_mcp_servers_enabled_tools_supports_raw_names(
    ...
    try:
        await connect_mcp_servers(
            {"test": _make_mcp_cfg(enabled_tools=["demo"])},
            registry,
            stack,
        )
    finally:
...
async def test_connect_mcp_servers_enabled_tools_defaults_to_all(
    ...
    try:
        await connect_mcp_servers(
            {"test": _make_mcp_cfg(enabled_tools=["*"])},
            registry,
            stack,
        )
    finally:
...
async def test_connect_mcp_servers_enabled_tools_supports_wrapped_names(
    ...
    try:
        await connect_mcp_servers(
            {"test": _make_mcp_cfg(enabled_tools=["mcp_test_demo"])},
            registry,
            stack,
        )
    finally:
...
async def test_connect_mcp_servers_enabled_tools_warns_on_unknown_entries(
    ...
    import agent_harness.mcp.client as mcp_client

    original_warning = mcp_client.logger.warning
    mcp_client.logger.warning = _warning

    stack = AsyncExitStack()
    await stack.__aenter__()
    try:
        await connect_mcp_servers(
            {"test": _make_mcp_cfg(enabled_tools=["unknown"])},
            registry,
            stack,
        )
    finally:
        await stack.aclose()
        mcp_client.logger.warning = original_warning
